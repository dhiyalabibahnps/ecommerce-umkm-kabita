...
<template>
  <router-link :to="`/produk/${props.product.slug}`">
    <div class="h-full overflow-hidden rounded md:rounded-sm border border-[#e1e2ed] bg-white shadow-sm">
      <div class="block overflow-hidden bg-surface-container">
        <div v-if="props.product.images?.[0]?.url" class="flex h-45.5 w-full items-center justify-center">
          <img :src="props.product.images[0].url!" :alt="props.product.name" class="h-full w-full object-cover" />
        </div>
        <div v-else class="flex h-45.5 items-center justify-center text-sm text-slate-400">Tidak ada gambar</div>
      </div>

      <div class="py-2 px-3">
        <div class="no-underline text-sm font-semibold text-slate-950 hover:text-slate-700">
          {{ props.product.name }}
        </div>
        <p class="text-xs text-slate-500 mt-2 mb-4">{{ props.product.shop?.name }}</p>
