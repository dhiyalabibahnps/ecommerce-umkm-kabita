      ...
      <template v-else>
                ...
                <div v-for="item in group.items" :key="item.id" class="p-4 flex items-start gap-3 sm:gap-4">
                  ...
                  <div class="flex-1 min-w-0 space-y-1">
                    <div class="flex items-start justify-between gap-2">
                      ...
                      <button @click="deleteItem(item.id)" class="text-slate-400 hover:text-rose-600 transition-colors">
        ...
        <div v-else class="rounded-3xl border border-slate-200 bg-white px-6 py-16 text-center shadow-sm">
          <div class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-blue-50 text-blue-600">
            <i class="pi pi-shopping-bag text-[48px]!"></i>
          </div>
          <h2 class="mt-5 text-xl font-bold text-slate-900">Keranjang masih kosong</h2>
          <p class="mx-auto mt-2 max-w-4xs text-sm leading-6 text-slate-500">
