...
<template>
  <section class="mx-auto max-w-6xl space-y-5 pb-12">
    ...
    <header class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <Button label="Segarkan" icon="pi pi-refresh" size="small" outlined class="rounded-xl!" :loading="loading"
        @click="loadOrders" />
    </header>

    <!-- Top Status Summary Filter Cards -->
    <div class="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-6">
      <button v-for="tab in summaryTabs" :key="tab.value" type="button"
        ...
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-medium leading-tight transition"
        ...
        <p class="mt-2 text-xl font-black transition tracking-tight"
    ...
    <div
      class="flex flex-col gap-2.5 rounded-xl border border-slate-200/80 bg-white p-3 shadow-2xs sm:flex-row sm:items-center">
      <div class="relative flex-1">
        <i class="pi pi-search absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-400"></i>
        <InputText v-model="search" placeholder="Cari nomor order, nama pembeli, atau produk..."
      ...
      <div class="flex items-center gap-2">
        ...
        <Button v-if="search || selectedStatus" label="Reset" icon="pi pi-times" text size="small"
          class="text-xs! text-slate-500 hover:text-slate-700" @click="search = ''; selectedStatus = ''" />
      </div>
    </div>

    <!-- Loading State -->
    <div v-if="loading"
      class="flex flex-col items-center justify-center rounded-2xl bg-white py-20 text-slate-400 shadow-2xs">
    ...
    <div v-else-if="filteredOrders.length === 0"
      class="rounded-2xl border border-slate-100 bg-white p-16 text-center shadow-2xs">
      ...
      <article v-for="order in filteredOrders" :key="order.id"
        ...
        <div class="grid grid-cols-1 gap-4 py-3 sm:grid-cols-12 sm:items-center">
          ...
          <div class="sm:col-span-6 flex items-start gap-3">
            <div v-if="order.items?.[0]?.product?.images?.[0]?.url"
              class="h-14 w-14 shrink-0 overflow-hidden rounded-lg border border-slate-100 bg-slate-50">
              <img :src="order.items[0].product.images[0].url" :alt="order.items[0].product.name"
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-[11px] text-slate-500 mt-0.5">
                {{ order.items?.length || 1 }} produk • Pembeli: <strong class="text-slate-700">{{ order.buyer?.name ||
                  'Pembeli' }}</strong>
              </p>
              <p v-if="order.notes"
                class="text-[10px] text-amber-700 bg-amber-50 rounded px-1.5 py-0.5 mt-1 inline-block border border-amber-200/60 line-clamp-1">
                Catatan: "{{ order.notes }}"
              </p>
            </div>
          </div>
          ...
          <div class="sm:col-span-3 text-right">
            ...
            <span class="text-[10px] text-slate-400 capitalize">{{ order.payment_method === 'transfer' ? 'Transfer Bank'
              : 'COD' }}</span>
          </div>
        </div>

        <!-- Specific Notice for awaiting_verification -->
        <div v-if="order.status === 'awaiting_verification'"
        ...
        <div class="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3">
          <div class="flex items-center gap-2">
            <Button label="Chat Pembeli" icon="pi pi-comments" severity="secondary" text size="small"
              class="text-xs! text-blue-600! hover:bg-blue-50!" @click="openChat(order)" />
          </div>

          <div class="flex items-center gap-2">
            <Button label="Detail" icon="pi pi-arrow-right" iconPos="right" severity="secondary" outlined size="small"
              class="text-xs! rounded-lg!" @click="router.push(`/seller/pesanan/${order.id}`)" />
            ...
            <template v-if="order.status === 'processing'">
              <Button label="Proses Pesanan" icon="pi pi-box" size="small"
                ...
                :loading="actionLoadingId === order.id" @click="handlePackOrder(order)" />
            </template>

            <template v-else-if="order.status === 'packed'">
              <Button label="Konfirmasi Pengiriman" icon="pi pi-send" size="small"
                class="bg-blue-600! border-blue-600! text-xs! rounded-lg! font-bold!"
                :loading="actionLoadingId === order.id" @click="openShipModal(order)" />
            </template>

            <template v-else-if="order.status === 'shipped'">
              <Button label="Lihat Pengiriman" icon="pi pi-truck" severity="info" outlined size="small"
                class="text-xs! rounded-lg!" @click="router.push(`/seller/pesanan/${order.id}`)" />
            </template>

            <template v-else-if="order.status === 'cod_meeting'">
              <Button label="Selesaikan COD" icon="pi pi-check" severity="success" size="small"
                class="text-xs! rounded-lg!" :loading="actionLoadingId === order.id"
    ...
    <Dialog v-model:visible="shipModalVisible" modal header="Konfirmasi Pengiriman Pesanan"
      ...
      <div class="space-y-4 pt-1">
        ...
        <div class="flex items-center justify-end gap-2 pt-4 border-t border-slate-100">
          ...
          <Button label="Kirim Pesanan" icon="pi pi-send" size="small"
            class="bg-blue-600! border-blue-600! rounded-lg! text-xs! font-bold!" :loading="isShipSubmitting"
